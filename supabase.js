...
// ── Inicialización ────────────────────────────────────────────────────────
// Supabase JS v2 UMD expone window.supabase.createClient directamente
const { createClient } = window.supabase;

if (typeof createClient !== 'function') {
  console.error('Supabase JS no cargado correctamente');
}

// Exponer URL al window para uso interno
...
const Auth = {
  ...
  /**
   * Registro completo: crea el usuario en auth.users y su fila en perfiles.
   * extra puede incluir: { apellido, telefono, rol, matricula_personal, matricula_personal_localidad }
   * rol por default es 'comprador' si no se especifica.
   */
  async registro(email, password, nombre, extra = {}) {
    ...
    if (!user) return { data, error: null }; // confirmación de email pendiente, perfil se crea al confirmar

    // Crear/actualizar fila en perfiles (idempotente con upsert)
    const perfilData = {
      id: user.id,
      nombre,
      ...
      email,
      rol: extra.rol || 'comprador',
      matricula_personal: extra.matricula_personal || null,
      matricula_personal_localidad: extra.matricula_personal_localidad || null,
    };
    const { error: perfilError } = await db.from('perfiles').upsert(perfilData);
    if (perfilError) console.error('Error creando perfil:', perfilError);

    return { data, error: perfilError || null, userId: user.id };
  },
  async logout() {
...
// ── Inmobiliarias — alta y certificación manual Pinamar ──────────────────
const Inmobiliarias = {
  async registrar(formData) {
    // formData: objeto con los campos del formulario inmobiliarias-registro.html
    // estado_alta queda en 'pendiente' por default (definido en la tabla)
    return db.from('inmobiliarias').insert(formData);
  },

  async subirLogo(inmobiliariaId, file) {
    const path = `${inmobiliariaId}/logo-${file.name}`;
    const { error } = await db.storage.from('inmobiliarias-docs').upload(path, file);
    if (error) return { error };
    const { data } = db.storage.from('inmobiliarias-docs').getPublicUrl(path);
    return { url: data?.publicUrl };
  },

  async subirPortada(inmobiliariaId, file) {
    const path = `${inmobiliariaId}/portada-${file.name}`;
    const { error } = await db.storage.from('inmobiliarias-docs').upload(path, file);
    if (error) return { error };
    const { data } = db.storage.from('inmobiliarias-docs').getPublicUrl(path);
    return { url: data?.publicUrl };
  ...
  /** Lista solo inmobiliarias certificadas, opcionalmente filtradas por zona */
  async listarCertificadas(zona = null) {
    let q = db.from('inmobiliarias')
      .select('id,nombre,logo_url,descripcion_corta,instagram,web,zonas_opera,certificado_pinamar')
      .eq('activa', true)
      .eq('certificado_pinamar', true);
    if (zona) q = q.contains('zonas_opera', [zona]);
    return q;
  },

  /** Cuenta propiedades activas reales de una inmobiliaria (dato siempre actualizado) */
  async contarPropiedadesActivas(inmobiliariaId) {
    const { count } = await db.from('propiedades')
      .select('id', { count: 'exact', head: true })
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  Auth.getUser().then(async user => {
    ...
    if (!navR) return;
    // Reemplazar SOLO el botón "Iniciar sesión" (btn-in) por el de Mi cuenta.
    // No tocar otros links que también apunten a login-registro.html
    // (ej: "+ Publicar"), para no duplicar el cambio.
    const btnLogin = navR.querySelector('.btn-in');
    if (btnLogin) {
      btnLogin.textContent = '👤 Mi cuenta';
      // Consultar el rol real antes de decidir el destino — un admin
      // debe ir a panel-admin.html, no asumir siempre inmobiliaria.
      try {
        const perfil = await Auth.getPerfil();
        btnLogin.href = perfil?.rol === 'admin' ? 'panel-admin.html' : 'panel-inmobiliaria.html';
      } catch(e) {
        btnLogin.href = 'panel-inmobiliaria.html';
      }
    }
  });
});
