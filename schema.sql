...
returns trigger language plpgsql as $$
declare anio text; seq bigint;
begin
  anio := to_char(now(), 'YYYY');
  select count(*)+1 into seq from public.propiedades
...
create policy if not exists "noticias_lectura_publica"
  on public.noticias for select
  using (publicada = true);

-- Política: admin puede todo
create policy if not exists "noticias_admin"
  on public.noticias for all
  using (exists (select 1 from public.perfiles where id = auth.uid() and rol = 'admin'));

-- Política: inmobiliaria autorizada puede insertar/editar sus propias noticias
create policy if not exists "noticias_inmobiliaria_autorizada"
  on public.noticias for all
  using (
    inmobiliaria_id in (
